...
export default function InfiniteMenu({ items = [], enableAutoNavigation = false }) {
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      // Stop animation and clean up WebGL resources
      if (sketch) {
        sketch.stop();
      }
    };
  ...
  useEffect(() => {
    ...
    if (!isMoving && activeItem?.link) {
      const targetLink = activeItem.link;
      
      // Skip if we already navigated to this link or if we're already at this location
      if (lastNavigatedRef.current === targetLink || location === targetLink) {
        return;
      }
      ...
      if (targetLink.startsWith('http')) {
        ...
        lastNavigatedRef.current = targetLink;
        setLocation(targetLink);
      }
    }
  }, [isMoving, activeItem, location, setLocation, enableAutoNavigation]);
    ...
    <div style={{ position: 'relative', width: '100%', height: '100%' }}>
      ...
      {activeItem && (
        <div className={`face-title ${isMoving ? 'inactive' : 'active'}`}>
          <div style={{ fontWeight: '700', fontSize: '1.5rem', marginBottom: '0.3em' }}>
            {activeItem.title}
          </div>
